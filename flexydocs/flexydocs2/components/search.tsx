...
export function AISearchTrigger() {
  ...
  // Debugging: log chat status and message count to help trace client updates
  useEffect(() => {
    console.log('[AISearchTrigger] chat status:', chat.status);
  }, [chat.status]);

  useEffect(() => {
    try {
      console.log(
        '[AISearchTrigger] messages.length:',
        Array.isArray(chat.messages) ? chat.messages.length : 0,
      );
    } catch (err) {
      console.log('[AISearchTrigger] messages debug error', err);
    }
  }, [chat.messages?.length]);

  // Log last message details for debugging (role + concatenated text parts)
  useEffect(() => {
    const last = Array.isArray(chat.messages) ? chat.messages.at(-1) : undefined;
    if (!last) return;
    ...
    try {
      ...
      console.log('[AISearchTrigger] last message role:', last.role);
      console.log('[AISearchTrigger] last message text:', text);
    } catch (err) {
      console.log('[AISearchTrigger] last message debug error', err);
    }
  }, [chat.messages?.length]);
